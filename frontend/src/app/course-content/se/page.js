...
const CourseContentPage = ({ courseName }) => {
  ...
  const practiceAssignment = {
    ...
    questions: [
      {
        question: "The acceptance testing conducted by a group of selected end users in a real-live environment is called___?",
        options: ["Unit Testig", "Integration Testing", "Beta Testing", "Alpha Testing"],
        correctAnswer: 0,
      },
      {
        question: "An EdTech company got a contract to build an E-learning system for a school. It decides to first focus on the key feature that the school requires - i.e. to create an online attendance module. The team develops, tests and delivers this module to the school, and then starts working on the next module. The software development process followed in this case is similar to?",
        options: ["Waterfall Model", "V Model", "Prototype Model", "Agile Model"],
        correctAnswer: 0,
      },
    ],
  };
  ...
  const gradedAssignment = {
    ...
    questions: [
      {
        question: "A software company wants to build a website for employee wellfare to cater services to its own employees, and thus requires to interact with the employees from different departments. In this case which type of clients are we dealing with?",
        options: ["external users", "internal users", "developers", "software or software components"],
        correctAnswer: 2,
      },
      {
        question: "Which of the following tests ensures that the requirements given by the clients are actually met?",
        options: ["Unit", "Modular", "Integration", "Acceptance"],
        correctAnswer: 2,
      },
    ],
  };
          ...
          {selectedAssignment && (
            <div className="bg-white p-4 shadow-lg rounded-lg">
              ...
              {selectedAssignment.questions.map((q, index) => (
                <div key={index} className="mb-4">
                  <p className="font-semibold text-black">{index + 1}. {q.question}</p>
                  {q.options.map((option, optionIndex) => (
                    <label key={optionIndex} className="block mt-1 text-black">
                      <input
                        type="radio"
                        name={`question-${index}`}
                        value={optionIndex}
